...
请求的页面不建实例（其驻留不影响代价）。本实现按用户指定的模型，对请求一次就不再请求的
死页（含仅出现一次的对象，以及多请求页末次请求之后的死区），假设其“下次请求”为真实
n 次请求之后的第 n+1 次，为末次请求 j（j≤n-1，无论此前是否请求过）建立**虚拟实例**
[j+1,n]：未调度 <=> 末次请求后驱逐；被调度 <=> 保留至 trace 结尾。虚拟实例参与
local-ratio 优化（与真实实例竞争空间与 penalty），但其 penalty 仅存在于模型（n+1 不在
真实请求序列内），**不计入真实 reload cost**；代价是算法的 4-近似保证只对“真实+虚拟”
的扩展目标成立，对真实 cost 不再有直接的 4-近似保证（真实 cost ≤ 扩展代价 ≤
4·OPT(扩展)）。
...
- evictions 口径（调度->驱逐的等价转换，两类驱逐均计数）：
  ...
  * 虚拟实例（死页，含仅请求一次者）未调度：末次请求后驱逐（模型事件）；被调度则
    保留至结尾、不驱逐（末次请求恰在 n 的页面与零大小页同样保留至结尾）。
...
class LocalRatioCaching:
    ...
    def solve(self, trace: Iterable[Tuple], capacity: int,
        ...
        #   * 虚拟实例（死页扩展）：末次请求 j 后无后续请求（nxt[j]==0）——含请求
        #     一次就不再请求的死页（j 同时也是首次请求）——假设“下次请求”为 n+1，
        #     区间 [j+1,n]（要求 j<=n-1，j==n 时区间空、必然保留）；
    ...
    def run(self, trace: Iterable[Tuple], capacity: int,
        ...
        #   (b) 虚拟实例（死页，含仅请求一次者，假设下次请求 n+1）未调度：末次请求后
        #       驱逐 -- 一次驱逐；被调度 / 末次请求恰在 n（区间空，必保留）/ 零大小页：
        #       保留至结尾，不驱逐。
